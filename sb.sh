...
user_db_cleanup_current_and_save_override() {
  return 0
}
...
apply_user_menu_overrides() {
  ...
  eval "$(declare -f user_db_cleanup_current_and_save_override | sed '1s/user_db_cleanup_current_and_save_override/user_db_cleanup_current_and_save/')"
}
...
${R}已安装核心模块如下（多个用 + 连接，如 1+2）:${NC}"
  ...
  elif user_db_exists && [ ${#removed_entry_keys[@]} -gt 0 ]; then
    local db_json removed_nodes_json
    removed_nodes_json="$(printf '%s\n' "${removed_entry_keys[@]}" | awk 'NF' | LC_ALL=C sort -u | jq -R . | jq -s '.')"
    db_json="$(user_db_load)"
